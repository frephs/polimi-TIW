...
public class AccountManager {
    ...
    public static class LoginManagerServlet extends ThymeleafHTTPServlet {
        ...
        public void doPost(HttpServletRequest request, HttpServletResponse response) {
            ...
            try {
                ...
                request.getSession().setAttribute("message", MessageType.WARNING.wrap(e.getMessage()));
            } catch (SQLException e) {
                request.getSession().setAttribute("message", MessageType.ERROR.wrap(e.getMessage()));
            } finally {
    ...
    public static class LogoutManagerServlet extends ThymeleafHTTPServlet {
        ...
        public void doPost(HttpServletRequest request, HttpServletResponse response) {
            ...
            request.getSession().setAttribute("message", MessageType.SUCCESS.wrap("User logged out successfully."));
    ...
    public static class DeleteAccountManagerServlet extends ThymeleafHTTPServlet {
        ...
        public void doPost(HttpServletRequest request, HttpServletResponse response) {
            try {
                userConnectionHandler.deleteAccount(
                        ((User) request.getSession().getAttribute("user")).getUsername(),
                        request.getParameter("password")
                );
                request.getSession().setAttribute("user", null);
                request.getSession().invalidate();
                request.getSession().setAttribute("message", MessageType.SUCCESS.wrap("Account deleted successfully"));
                sendRedirect(request, response, "/");
            } catch (SQLWarning e) {
                request.getSession().setAttribute("message", MessageType.WARNING.wrap(e.getMessage()));
                sendRedirect(request, response, "/account");
    ...
    public static class UpdateUsernameServlet extends ThymeleafHTTPServlet {
        @Override
        public void doPost(HttpServletRequest request, HttpServletResponse response) {
            if (request.getParameter("new-username").equals(
                    ((User) request.getSession().getAttribute("user")).getUsername()
            )) {
                request.getSession().setAttribute("message", MessageType.WARNING.wrap("New username must be different from the old one"));
            } else if (request.getParameter("new-username").isEmpty()) {
                request.getSession().setAttribute("message", MessageType.WARNING.wrap("New username cannot be empty"));
            } else if (request.getParameter("password").isEmpty()) {
                request.getSession().setAttribute("message", MessageType.WARNING.wrap("Password cannot be empty"));
            } else {
                try {
                    ...
                    request.getSession().setAttribute("user",
                            userConnectionHandler.getUser(
                                    request.getParameter("new-username"),
                                    request.getParameter("password")
                            )
                    );
                    request.getSession().setAttribute("message", MessageType.SUCCESS.wrap("Username updated successfully"));
                } catch (SQLWarning e) {
                    request.getSession().setAttribute("message", MessageType.WARNING.wrap(e.getMessage()));
                } catch (SQLException e) {
                    request.getSession().setAttribute("message", MessageType.ERROR.wrap(e.getMessage()));
    ...
    public static class UpdatePasswordServlet extends ThymeleafHTTPServlet {
        @Override
        public void doPost(HttpServletRequest request, HttpServletResponse response) {
            if (!request.getParameter("new-password").equals(request.getParameter("confirm-new-password"))) {
                request.getSession().setAttribute("message", MessageType.WARNING.wrap("New password and confirmation do not match"));
            } else if (
                ...
                try {
                    ...
                    request.getSession().setAttribute("message", MessageType.SUCCESS.wrap("Password updated successfully"));
                } catch (SQLWarning e) {
                    request.getSession().setAttribute("message", MessageType.WARNING.wrap(e.getMessage()));
                } catch (SQLException e) {
                    request.getSession().setAttribute("message", MessageType.ERROR.wrap(e.getMessage()));
    ...
    public static class UpdateManagerServlet extends ThymeleafHTTPServlet {
        ...
        public void doPost(HttpServletRequest request, HttpServletResponse response) {
            if (
                ...
                request.getSession().setAttribute("message", MessageType.WARNING.wrap("All fields must be filled"));
            } else {
                try {
                    if (Integer.parseInt(request.getParameter("street-number")) < 1) {
                        request.getSession().setAttribute("message", MessageType.WARNING.wrap("Street number must be a positive number"));
                    } else if (Integer.parseInt(request.getParameter("zip-code")) < 10000 || Integer.parseInt(request.getParameter("zip-code")) > 99999) {
                        request.getSession().setAttribute("message", MessageType.WARNING.wrap("Zip code must be a positive 5 digit number "));
                    } else {
                        ...
                        request.getSession().setAttribute("message", MessageType.SUCCESS.wrap("Account details updated successfully"));
                        request.getSession().setAttribute("user", new_user);
                    }
                } catch (SQLException e) {
                    request.getSession().setAttribute("message", "Something went wrong while updating your details. " + MessageType.ERROR.wrap(e.getMessage()));
                } catch (NumberFormatException e) {
                    request.getSession().setAttribute("message", MessageType.WARNING.wrap(
                            "Street number and zip code must be numbers"
                    ));
                }
            }
            sendRedirect(request, response, "/account");
